...
public class NotaryImpl implements Notary {
	...
	private static int SERVER_DATA_EXPIRY_IN_MS = Integer.parseInt("300000");
	...
	private NotaryImpl() {
		...
		try {
			...
			DDB_REGION = "eu-west-1";//System.getenv("DDB_REGION");
			secretName = "/environment/dev3/fido-secret";//System.getenv("SECRET_KEY_NAME");
			SIGNATURES_TABLE_NAME = "fidosignatures";//System.getenv("SIGNATURES_TABLE_NAME");
			...
			ddb = AmazonDynamoDBClient.builder().withCredentials(new ProfileCredentialsProvider("myProfile")).withRegion(DDB_REGION).build();
	...
	public String sign(String signData) {
		...
		try {
			String signature = Base64.encodeBase64URLSafeString(HMAC.sign(signData, "washdefrioiugiuudfvdruhiueesrvskjdfhvsetiu"));
			storeAWS(signature);
			return signature;
		} catch (Exception e) {
			logger.error(e);
...
/*public static void main(String[] args) {
	System.out.println(Base64.encodeBase64URLSafeString(HMAC.sign("", secretHelper.getCurrent("/environment/dev3/fido-secret"))));
}*/
}
